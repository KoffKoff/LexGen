...
dfaOut dfa = start ++ newFun "start_state" [] "SNum" ++ show start_state ++ "\n" ++
  ...
  where start_state:_ = dfa'_start_states dfa
        automata = show_dfa $ dfa'_states dfa
        start = createBlockComment "The automata it's big :("
...
show_dfa :: IM.IntMap (Edges SNum Code) -> String
show_dfa dfa = "IM.fromList [" ++ drop 1 (IM.foldlWithKey show_edges "" dfa) ++ "]" -- mer skit
  where show_edges str char edges = str ++ ",(" ++ show char ++ ",Map.fromList ["
                                    ++ drop 1 (M.foldlWithKey show_edge "" edges) ++ "])"
        show_edge str is (os,accs) = str ++ ",(" ++ show is ++ ",(" ++ show os ++ ",[" ++
                                     drop 1 (foldl show_acc "" accs) ++ "]))"
        show_acc str (Acc p act lctx rctx) =  str ++ ",Acc " ++ show p ++ " " ++ show_act act
                                              ++ " " ++ show lctx ++ " " ++ show rctx
        show_act (Just act) = "(Just " ++ show act ++ ")"
        show_act Nothing = "Nothing"