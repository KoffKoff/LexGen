...
\section{Fingertree}
The fingertree is constructed with the characters of a text being the leafs and
with the table format transition map as it's measure. The $Table$ data type has
to be a monoid in order to be a legal measure of the fingertree.

In order for the table
data type to be a legal measure of the fingertree the table data type has to be
a monoid.
...
the tokens generatd are valid, the tokens are passed on to $combineWithRHS$
...
\item[otherwise]If $toks2$ is not valid the lexer checks the suffix of $toks1$
  ...
  \item If the $Str$ constructor is found the suffix doesn't end in a valid
...
the first token of the second tokens, for variable references see
\cref{fig:mergetokens}.
...
\item If there is exactly one token in $toks2$, the suffix from $toks1$ is
  combined with $suff1$. When two suffixes are combined some extra checks needs
  to be done. If $toks2$ has an accepting out state, the two suffixes
...
$mergeSuff$ checks which pair of suffix it has and takes the appropriate
actions.
