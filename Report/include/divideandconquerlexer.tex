...
into smaller part and analyse them; and then combining them. In the base
...
This section will try to give an idea of how the Divide and Conquer algorithm
works in a general case. So when talking about the incremental lexer there is an
understanding about the underlying techniques.
...
and Conquer algorithm always consist of a pattern with these three steps \cite{Goodrich}.
\begin{description}
...
In divide and conquer algorithms this is essential. In the conquer step of the
divide and conquer algorithm, there is no certain order of how the subproblems
are going to be conquered. This means that the order the subproblems are being
conquered can't matter for the algorithm, hence the conquer step must be
associative.
...
\end{center}
This is typically the equation of a divide-and-conquer algorithm, where $\alpha$
...
that solves different problems. A general problem is sorting, or more precies
sorting a sequence of integers. This example will show merge-sort.
...
using the master method. There are $2$ recurrsive calls, the subproblems are
$1/2$ of the original problem size. To merge the two sorted subproblems the
worst case is to check every element in the two list, $2 \cdot n/2 = n$.
\begin{center}
 $T(n) = 2T(n/2) + n$.
\end{center}
Results in: 
